...
export default function MyCasesPage() {
  ...
  const displayedCases = filteredCases.slice(startIndex, startIndex + itemsPerPage);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayedCases.length > 0 ? (
            displayedCases.map((c) => (
              ...
              >
                <h2 className="font-semibold text-lg">{c.name || 'Untitled Case'}</h2>

                <p
                  className={`text-sm mt-2 mb-4 text-gray-400`}
                >
                ...
                <div className="text-sm space-y-1">
                  <div className="flex items-center">
                    ...
                    <span className='text-gray-400'>Created {formatDate(c.created_at)}</span>
                  </div>
                  <div className="flex items-center">
                    <Clock className="inline w-4 h-4 mr-1 text-gray-400" />
                    <span  className='text-gray-400'>Updated {formatDate(c.updated_at)}</span>
                  </div>
                </div>
            ...
            <p className="text-center text-gray-500 col-span-full">No cases found.</p>
          )}
        </div>
      )}
